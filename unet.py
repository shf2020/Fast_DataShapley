...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.out=nn.Linear(14*14,10)
    def forward(self, x):
        y = self.conv(x).view(self.conv(x).size(0), self.class_num, -1)
        output=self.out(y)
        return output
...
class UNet(nn.Module):
    def __init__(self,
        ...
        # Output layer.
        self.outc = OutConv(out_channels, n_classes) #128
